export function Marquee({ items }: { items: string[] }) {
  const row = (
    <ul className="marquee-track items-center gap-10 pr-10" aria-hidden="true">
      {items.map((item, i) => (
        <li key={i} className="flex items-center gap-10 whitespace-nowrap text-sm font-bold uppercase tracking-wide text-ink/40">
          <span>{item}</span>
          <span className="w-2 h-2 bg-red" style={{ transform: "rotate(45deg)" }} />
        </li>
      ))}
    </ul>
  );
  return (
    <div className="marquee relative overflow-hidden py-5 border-y-[3px] border-ink/[0.06]">
